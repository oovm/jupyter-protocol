...
pub(crate) fn split_into_statements(code: &str) -> Vec<OriginalUserCode> {
        ...
        .and_then(ast::BlockExpr::stmt_list)
    {
        ...
        while let (Some(child), next) = (children.next(), children.peek()) {
            ...
            let end = next.map(|next| usize::from(next.text_range().start()) - prelude.len()).unwrap_or(code.len());
            output.push(OriginalUserCode { code: &code[start_byte..end], node: child, start_byte });
            start_byte = end;
        }
    }
    output
...
mod test {
    ...
    fn split_and_get_text(code: &str) -> Vec<&str> {
        split_into_statements(code).into_iter().map(|orig| orig.code).collect()
    }

    #[test]
    fn single_line() {
        assert_eq!(split_and_get_text("let mut a = 10i32; a += 32;"), vec!["let mut a = 10i32; ", "a += 32;"]);
    }

    #[test]
    fn multiple_lines() {
        assert_eq!(split_and_get_text("let mut a = 10i32;\n  foo()"), vec!["let mut a = 10i32;\n  ", "foo()"]);
    }

    #[test]
    fn statement_ends_with_brace() {
        assert_eq!(split_and_get_text("if a == b {foo(); bar();} baz();"), vec!["if a == b {foo(); bar();} ", "baz();"]);
    }
    ...
    fn else_statements() {
        assert_eq!(
            ...
            vec!["if a == b {foo(); bar();}  else if a < b {baz();} else {foo();} ", "b"]
        );
    }
